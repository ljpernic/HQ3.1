              ...
              .map(({ node: post }) => {
                return (
                  <div className="container" key={post.id}>
                      ...
                      <h2>By <Link to={post.frontmatter.author.idpath}> {post.frontmatter.author.id}</Link> in  (issue name){/*<Link to={post.frontmatter.issue.idpath}> {post.frontmatter.issue.id}</Link>*/}</h2>
                      <p>{post.excerpt}</p>
                      <hr />
                  </div>
                )
